  ...
  return (
    <section>
      ...
      <Link href="/inventory" className="loadout-preview group">
        <span className="loadout-equipped"><Check size={12} />{t("home_shortcutInventory")}</span>
        ...
        <div className="loadout-caption relative flex items-center justify-between gap-2 px-4 pb-4"><span className="min-w-0"><strong className="block text-sm text-[rgb(var(--c5))] truncate">{cardName}</strong><span className="block mt-1 text-xs text-[rgb(var(--c4))] truncate">{tableName}</span></span><ArrowUpRight size={17} /></div>
      </Link>
    </section>
  );
}
